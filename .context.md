...
- **browser-cookie3**: Auto-extract cookies from browser
...
│   ├── utils.py                 # Utility functions (sanitization, validation, I/O)
│   ├── verificator.py           # Core verification logic
│   ├── auto_extract.py          # 🆕 Smart credential auto-extraction
│   └── web/
│       ├── __init__.py
│       ├── app.py               # Flask application
│       └── templates/
│           └── index.html       # Web UI with modern gradient design
│
├── tests/
│   └── sample_codes.txt         # Sample test data
│
...
├── QUICKSTART_ID.md            # Quick start guide (Indonesian)
├── SETUP_ID.md                 # Setup guide (Indonesian)
├── AUTO_EXTRACT_ID.md          # 🆕 Auto-extraction feature docs
...
- Auto-loaded credentials (BANDCAMP_CRUMB, BANDCAMP_CLIENT_ID, BANDCAMP_SESSION)
...
**API Endpoint**: `POST /api/codes/1/verify`  
**Required Headers**: Cookie (client_id, session), X-Requested-With, Referer  
**Required Payload**: `crumb`, `code`

### 5. Auto-Extraction (app/auto_extract.py) 🆕
...
2. **Crumb Scraping**: Fetches bandcamp.com/yum and extracts crumb from HTML/JavaScript
3. **Auto-Fill**: Returns credentials for automatic form population

**Methods**:
- `extract_from_browser(browser_name)`: Extract cookies
- `extract_crumb_from_page()`: Scrape crumb from Bandcamp
...
| `/api/auto-extract` | POST | 5/min | 🆕 Auto-extract credentials |
...
- 📝 Text area for code input (max 1000 codes)
- 🤖 **Auto-Extract Button** - One-click credential extraction
- 🔄 Real-time AJAX progress updates
- 📊 Live results table with animations
- 📥 CSV export functionality
- 🚦 Status indicators and progress bar

**UX Flow**:
1. User clicks "Auto-Extract" → system reads browser cookies → form auto-fills
2. User pastes codes → clicks "Start Verification"
...
### CLI Flow
```
User → cli.py → read codes.txt → BandcampVerificator 
  → verify_batch() → API calls with delays 
  → results → export_to_csv() → results.csv
```

### Web Flow
```
User → Browser → /api/auto-extract → CredentialExtractor 
  → browser cookies → auto-fill form
  
User → paste codes → /api/verify (AJAX per code) 
  → BandcampVerificator → verify_code() 
  → JSON response → update UI → repeat
  
User → Export CSV → client-side generation → download
```
...
User clicks "Auto-Extract" 
  ...
  → CredentialExtractor.extract_from_browser()
    → browser_cookie3 reads Chrome/Firefox/Edge cookies
    → finds client_id & session cookies
  → CredentialExtractor.extract_crumb_from_page()
    → requests.get(bandcamp.com/yum) with cookies
    → BeautifulSoup parses HTML
    → regex finds crumb in script tags
  → Return {crumb, client_id, session}
  → JavaScript fills form fields
...
BANDCAMP_CLIENT_ID=2F468B341CC6977036E49EBB0B6BB3A621721E8E...
...
**Method 1: Auto-Extraction (Recommended)** 🆕
...
5. Copy `client_id` and `session` from Cookies
...
**New Feature**: Smart Auto-Extraction 🤖
